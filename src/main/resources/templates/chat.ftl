...
<div class="qqBox">
    <div class="context">
        ...
        <div class="conRight">
            <div class="Righthead">
                <div class="headName"><b>聊天室</b></div>
            </div>
            <div class="RightCont">
                <ul class="newsList" id="message">
                    <#list recentRecords as record>
                        <li>
                            <div class="nesHead">
            ...
            <div class="RightMiddle">
                <div class="file">
                    <td>
                        <a href="/chatRecords"><font size="4" color="#00bfff">查找聊天记录</font></a>
                    </td>

                    <#--<img src="/img/file.jpg" height="30px" width="30px" onclick="/chatRecords">-->
                    <#--<form id="form_photo" method="post" enctype="multipart/form-data"-->
                        ...
                        <#--style="border-radius: 5px"></button> &ndash;&gt;-->
                    <#--</form>-->
                </div>
...
<script type="text/javascript">
    ...
    if ('WebSocket' in window) {
        webSocket = new WebSocket('ws://192.168.1.104:8090/webSocket?username=' + '${username}' + '&headUrl=' + '${headUrl}');
    } else {
    ...
    function getJson(onSuccess) {
        ...
        $.ajax({
            "async": true,
            "crossDomain": true,
            url: "http://localhost:8090/api/upload",
            type: "post",
    ...
    function send() {
        ...
        if (message.search("是否发送图片：") == 0) {
            //发送图片
            // $("#fasongPhoto").submit();
            var htmlstr = '<li><div class="answerHead"><img src=${headUrl}></div><div class="answers">'
                    + '<img id="jsonImg">' + '</div></li>';
            //获取接收方图片url
            getJson(function (re) {
    ...
    function setMessageInnerHTML(innerHTML) {
        if (innerHTML.indexOf("是否发送图片：") != -1) {
            ...
            if(innerHTML.indexOf("#") != -1){
                var headUrl = innerHTML.substring(0, innerHTML.indexOf("#"))
                var message = innerHTML.replace(headUrl + "#", "")
                var msg = '<li>'
                        + '<div class="nesHead">'
                        + '<img src='+headUrl+'>'
                        + ' </div>'
